namespace LinguisticTools.TextCat
{
    ...
    public class CharacterNGramExtractor : IFeatureExtractor<TextReader, string>, IFeatureExtractor<char[], string>, IFeatureExtractor<string, string> 
    {
        ...
        public CharacterNGramExtractor(int maxNGramLength, long maxLinesToRead = long.MaxValue)
        {
            ...
            this._maxLinesToRead = maxLinesToRead;
        }
        ...
        public IEnumerable<string> GetFeatures(string text, bool toLower = false)
        {
            return this.GetFeatures(new StringReader(text), toLower);
        }
        ...
        /// <param name="text"></param>
        /// <returns>the sequence of ngrams extracted</returns>
        public IEnumerable<string> GetFeatures(char[] text, bool toLower = false)
        {
            return this.GetFeatures(new string(text));
        }
        ...
        /// <param name="text"></param>
        /// <returns>the sequence of ngrams extracted</returns>
        public IEnumerable<string> GetFeatures(TextReader text, bool toLower = false)
        {
            long numberOfLinesRead = 0;
            var currentNgrams = Enumerable.Range(0, this._maxNGramLength).Select(_ => new Queue<char>()).ToArray();
            ...
            while ((charsRead = text.Read(buffer, 0, buffer.Length)) > 0)
            {
                for (int i = 0; i < charsRead; i++)
                {
                    ...
                    if (currentByte == 0xD || currentByte == 0xA && previousByte != 0xD)
                        numberOfLinesRead++;
                    if (numberOfLinesRead >= this._maxLinesToRead)
                        break;
                    ...
                    foreach (var ngram in this.UpdateAndProduceNgrams(charsToProcess, currentNgrams))
                        yield return ngram;
